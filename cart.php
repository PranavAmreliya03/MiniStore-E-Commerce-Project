...
<div class="cart_body">
    ...
    <div class="cart" style="display:flex; flex-direction:column; align-items:center;">
        <table align="center" style="border-collapse:collapse; text-align:center; width:80%;">
            <tr style="background-color:#2196f3; color:white;">
                <th colspan="2" style="padding:10px;">Description</th>
        ...
        <?php
            ...
            if(mysqli_num_rows($sql) == 0){
                echo "</table><h4 style='text-align:center; color:#999;'>You have nothing added to your cart</h4>";
            }
            else{
                    ...
                    if($row = mysqli_fetch_assoc($query)){?>
                        <tr>
                            <td style="padding:10px;">
                                <img src="images/<?=$row['img_url']?>" alt="" style="width:80px; height:80px; object-fit:cover;">
                            </td>
                            <td style="padding:10px;"><p><?=$row['product']?></p></td>
                            <td style="padding:10px;"><?=$p['qty']?></td>
                            <td style="padding:10px;">
                                <form action="removecart.php?id=<?=$row['pid']?>" method="post">
                                    <input type="submit" value="Remove" name="rem" 
                                    style="background-color:#f44336; color:white; border:none; padding:6px 10px; cursor:pointer; border-radius:5px;">
                                </form>
                            </td>
                            <td style="padding:10px;"><p>Rs. <?=$p['price']?></p></td>
                        </tr>
                    <?php
                    }
                }
        ...
        <?php
            $sql = mysqli_query($conn,"SELECT DISTINCT * FROM cart WHERE username = '$user' AND status = 'pending'");
            if(mysqli_num_rows($sql) != 0){ ?>
                <p style="text-align:center; margin-top:15px;">Total: 
                    <?php 
                ...
                <button type="submit" onmouseup="return purchaseCart()" 
